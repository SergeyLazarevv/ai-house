...
def format_tools_for_llm(tools: list[dict[str, Any]]) -> str:
    parts = [
        "## Каталог инструментов GitLab MCP",
...
class GitLabConnector(BaseConnector):
    ...
    async def connect(self, stack: AsyncExitStack) -> None:
        ...
        try:
            ...
            raise RuntimeError("Для GitLab MCP нужен пакет `mcp`. Установите зависимости заново.") from exc

        params = StdioServerParameters(
            command=sys.executable,
            args=["-m", "app.mcp_servers.gitlab"],
            env=os.environ.copy(),
        )
